...
REMOTE="contabo_vps:/opt/workhub/cookies.txt"
...
# BatchMode=yes: fail fast instead of hanging on an interactive password
# prompt if key auth is ever misconfigured — never silently blocks waiting
# for input in an unattended run.
scp -o BatchMode=yes "$COOKIES_FILE" "$REMOTE"
